...
class RecipeManagerCard extends LitElement {
  ...
  async _loadShoppingLists() {
    try { this._shoppingLists = (await this._api.getShoppingLists())?.lists ?? []; }
    catch { this._shoppingLists = []; }
  }
  ...
  async _handleAddToShopping(e) {
    ...
    try {
      ...
      if (res.filter(r => !r.success).length) console.warn('Some ingredients failed to add');
    ...
    return html`
      <nav class="rm-sidebar">
        ...
        <div class="sb-nav">
          ...
          ${navItem('mdi:cart-outline', 'Shopping List', 'shopping', true)}
